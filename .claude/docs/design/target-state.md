...
|---|---|
| **Large Language Model API key** | **Required:** `OPENAI_API_KEY`. **Optional and free:** `GROQ_API_KEY`, and what it is for — a second registered provider, which no published figure depends on. Say that these two providers are the whole of what Veritas talks to, that the key goes in `.env` (`.env.example` is the template), and that no data source needs a key. |

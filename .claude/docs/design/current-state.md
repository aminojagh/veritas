...
**Last updated:** 2026-08-28 — **Step 005 — Build the Validation Gate — is `active`,
and four of its five Sub-steps are built.** `veritas/validation/` refuses anything that
...
Restricted Column the Access Profile forbids, and refuses any statement that computes a
Certified Metric across joins its Metric Definition does not name or over a date column
it is not certified against; the rule that requires the Access Profile's predicate does
not exist yet. The Semantic Layer is
...
[ADR-0003](../adr/0003-validation-gate-is-deterministic-code.md). All four built
Sub-steps are ruled; `Route` is a registered Glossary term as of the same day.

---

## Resume here

- **Next: Sub-step 5.5 — the Gate requires the Access Profile's predicate, admits a
  slice route, and pays DEBT-020.** It is the last Sub-step of the Step, the only one
  that changes `semantic/`, and the largest;
  [R5](../plan/step-005-validation-gate.md#r5--55-is-a-pre-agreed-split-point--approved-by-amino-2026-08-25)
  pre-agreed it as the split point if the Step grows, and
  [R15](../plan/step-005-validation-gate.md#r15--aminos-rulings-on-the-54-review--decided-2026-08-28)
  says where the split falls if it fires. All four earlier Sub-steps are built and ruled
  —
  ...
  and R15. The
  [5.4 review entry](../reviews/step-005-validation-gate.md#sub-step-54--pay-debt-014-the-gate-checks-the-route-and-the-date-predicate)
  is the handoff detail. **Five declared probe verdicts 5.5 moves**, each declared today
  so the move is a measurement: `net revenue by region` in `traces.py` and in
  `route.py`, and `the name in a comment` and `the name in a filter only` in
  `restricted.py`, all four rejected on joins nothing certifies and all four flipping
  back when 5.5 certifies those hops; and `Realised P&L with its filter dropped` in
  `route.py`, allowed today and rejected once DEBT-020 is paid.
- **The [Step 005 plan](../plan/step-005-validation-gate.md) is `active`**, written and
  approved 2026-08-25 with **fifteen rulings** — ten of that date,
  [R11](../plan/step-005-validation-gate.md#r11--aminos-rulings-on-the-trim--decided-2026-08-26)
  and R12 of 2026-08-26, R13 and R14 of 2026-08-27, and
  [R15](../plan/step-005-validation-gate.md#r15--aminos-rulings-on-the-54-review--decided-2026-08-28)
  of 2026-08-28. Read the plan for what each decided.
  ...
  query can reach — which the Gate now says out loud, by rejecting a slice by it),
  ...
  projection walker, the restricted-column detector and the route reader from
  `veritas/validation/` and holds no copy of any of them. What stays in the spike is its
  three pinned declarations — three certified expressions, three certified routes and one
  Restricted Column.
- **Three of this Step's Ledger entries are settled and one is new.**
  ...
  fired and were paid in 5.4.
  [DEBT-020](../debt-ledger.md#debt-020--the-gate-checks-a-metrics-route-and-not-its-certified-filters)
  and [DEBT-021](../debt-ledger.md#debt-021--two-joins-to-one-table-under-different-aliases-are-not-told-apart)
  were both opened by 5.4 and neither Trigger fires in this Step — both name the Sub-step
  that builds Grounding. **DEBT-020 is paid in 5.5 anyway**, by
  [R15](../plan/step-005-validation-gate.md#r15--aminos-rulings-on-the-54-review--decided-2026-08-28):
  the route rule reads a Metric Definition's `join_paths` and `date_column` and not its
  `filters`, so `Realised P&L` with its certified predicate dropped is allowed today, and
  `check_validation_gate/route.py` declares that as a passing probe and prints what the
  omission is worth. **DEBT-021 stays open past this Step**: two joins to one table under
  different aliases are not told apart, so one statement computing two metrics that
  convert by different routes can cross the two conversions over and pass every rule.
  Nothing in the repository demonstrates it, and the Sub-step that pays it owes a probe.
  [DEBT-012](../debt-ledger.md#debt-012--the-price-table-is-sparse-so-the-snapshot-calendar-has-holes)
  came into reach in 5.4 and stays unfired: every date in the Gate's probes is read from
  the Snapshot calendar.
...
carry a Restricted Column, and anything that computes a certified metric over rows its
own Metric Definition does not describe.** `veritas/validation/` holds the
...
and seven rules behind them. Four judge a statement's
...
traces to names, and the date columns its WHERE clause keys on with that metric's
`date_column`, so a certified expression converted through the wrong currency or filtered
on the wrong half of a Section C date pair is refused. Permission comes from a list and
the Gate never searches for a route, so a slice that reaches a certified axis through
joins no entry names is refused too.
Each rejection carries its own named reason, ten in the taxonomy. The scan estimate and
the column list both come from the
engine through the Warehouse Adapter, which is the only place the plan format and the
catalogue query are known. **One rule does not exist**, so the Gate cannot yet require
the Access Profile's predicate — the profile carries a role and its Restricted Columns
and no permitted region — and nothing anywhere executes a statement through it.
...
│   │   └── gate.py            # the rules, the tracer, the lineage walk and the
│   │                           # route reader; one of the Gate's five decisions
│   │                           # is still missing
│   ├── warehouse/
...
**Everything above the Semantic Layer, and one fifth of the Validation Gate.** The
...
**Open debt: 11.** The one the current Step still reads first is
[DEBT-014](../debt-ledger.md#debt-014--the-spike-allows-a-query-the-gate-must-reject)
...
whose trigger — the Sub-step that builds Grounding — this Step does not fire, and which
[DEBT-020](../debt-ledger.md#debt-020--the-gate-checks-a-metrics-route-and-not-its-certified-filters)
shares.

...
Sub-step is built: the OpenAI default model is `gpt-5.4-mini`, chosen by measuring four
candidates, and it answers every answerable Gold Question correctly.**
...
- **Sub-step 8.1 is built and awaits Amino's ruling. Next is Sub-step 8.2 — Lineage
  records what the statement used.** 8.1 reverted its own failed first attempt, measured
  ...
  **failed its own runner** — Groq's free tier is capped at 200,000 tokens per day, the
  ...
  on 2026-09-03, five Sub-steps: 8.1, built and described above; Lineage read off what
  the statement used
...
- **No Term Proposal is open**: the plan's two —
  ...
  **`Gold Question`** and **`Relevant Set`** were agreed on 2026-09-01, and 7.2's
  Section D column the same day. **The debt picture after 8.1:**
  [DEBT-035](../debt-ledger.md#debt-035--a-composed-certified-metric-has-no-statement-the-gate-allows)
  ...
  is **closed `accepted`** by this Sub-step: its remedy was measured not to work over
  ...
  documentation pass needs it. `DEFAULT_PROMPT_FORM` stays `rules`, now on a margin

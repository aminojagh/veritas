...
**Last updated:** 2026-08-24 — **the corpus can say that a word has two meanings.** Sub-step 4.4 of Step 004 is written and verified: `semantic/ambiguous/` publishes the five rows of [Glossary Section D](../glossary.md#d-ambiguous-terms) as Ambiguous Term entries — "revenue", "volume", "balance", "P&L", "how much does X have" — each naming the Certified Metrics it disambiguates between. **This is the entry type [ADR-0001](../adr/0001-semantic-layer-as-the-retrieval-corpus.md) rejected schema retrieval over**, because a schema listing *"cannot represent the one fact that matters: that 'revenue' has two certified meanings"*. `check_semantic_layer.py` went from eleven checks to **fourteen**, and the three new ones are the only ones in the file that execute nothing: an Ambiguous Term is a claim about **language**, so it can be wrong while every expression in the corpus is right. **The third of four entry types is complete, and so are the other two** — nine Metric Definitions, eight Join Paths, five Ambiguous Terms, twenty-two entries in all. Nothing under `semantic/metrics/` or `semantic/joins/` was touched. **One Glossary amendment came out of it, and the check is what found it**: Section D's "P&L" row read `Realised · Unrealised`, spelling *neither* of the two Section B terms as registered, and now reads `Realised P&L · Unrealised P&L · both`. **Amino ruled it in on 2026-08-24**, together with the four other things 4.4 put up
sceptically and the prefix question R9 had left open — six rulings, recorded as
[R10](../plan/step-004-semantic-layer.md#r10--aminos-rulings-on-the-44-review--decided-2026-08-24), leaving one question open: whether the `resolution` field name wants a
Glossary row. The open-debt count is unchanged at **9** — 4.4 took no shortcut, and the [4.4 review](../reviews/step-004-semantic-layer.md#sub-step-44--write-the-ambiguous-terms) says why its two candidates are neither debt nor extension. Step 004 is `active`, approved by Amino on 2026-08-21 with all seven of its rulings, amended on 2026-08-22 by [R8](../plan/step-004-semantic-layer.md#r8--the-route-a-metric-definition-carries--decided-in-sub-step-42-under-aminos-ruling-of-2026-08-22) and again on 2026-08-23 by [R9](../plan/step-004-semantic-layer.md#r9--aminos-four-rulings-on-the-42-review--decided-2026-08-23); **Sub-step 4.5 — the Dimension Definitions — is the next and last commit of this Step.** **The Warehouse is full: all ten tables of Glossary Section B hold rows, every Certified Metric can return a number, and the adapter seam is checked in all three of the readings ADR-0002's two signals now come to.**

**Steps completed:** Step 000 (framework), Step 001 and **Step 002, all six Sub-steps, fully committed**. Step 000 and Sub-step 1.1 in `6281e6b`, Sub-step 1.2 in `4b48a46`, Sub-step 1.3 in `9c5b060`, Step 002 planning in `57e8aee`, Sub-step 2.1 in `5a061a7`, the R16 plan amendment in `cd5e7dd`, Sub-step 2.2 in `0fc5a34`, Sub-step 2.3 in `a58ef91`, Sub-step 2.4 in `13b99bb`, Sub-step 2.5 in `ce2961a`, Sub-step 2.6 in `6a16d3d`, Step 003 planning in `40d72d8`, **Sub-step 3.1 in `d840fa8`, Sub-step 3.2 in `89fee55`, Sub-step 3.3 in `23020e9`, Sub-step 3.4 in `c20d601`, Sub-step 3.5 in `fcf4b7d`**. **Step 004 planning is committed in `5d95393`** and **Sub-step 4.1 in `6c15736`**, which is what wrote 3.5's hash above and turned the [Step 003 plan](../plan/step-003-validation-feasibility.md) from `in review` to `done` — how every hash above arrived, and how `40d72d8` closed Step 002. **Sub-step 4.2 is committed in `333d6fc` and Sub-step 4.3 in `ae75f0e`. This commit is Sub-step 4.4**; its own hash is not here for the reason every Sub-step's is not, which is that this file is part of it, and the Sub-step 4.5 commit fills it in.

---

## Resume here

- **Next: Sub-step 4.5 — write the Dimension Definitions.**
  [The plan](../plan/step-004-semantic-layer.md#45--write-the-dimension-definitions)
  asks for the certified axes — by date, by region, by instrument type — as
  `semantic/dimensions/` entries, with the columns and allowed values the
  [`Dimension Definition`](../glossary.md#a-the-system) row already names, plus two
  assertions in `check_semantic_layer.py`: each named column exists in the live
  schema, and its **allowed values match what the Warehouse actually holds** — a
  Dimension Definition promising four instrument types over a Warehouse holding three
  is a certified axis that lies. Its verification is
  `uv run python .claude/scripts/check_semantic_layer.py` plus the mutation the plan
  names — add a fourth region to the allowed values, see the run fail against the
  Warehouse's three, restore, `cmp`.
  **This is the last Sub-step of Step 004**, and it is also
  [R5](../plan/step-004-semantic-layer.md#r5--45-is-a-pre-agreed-split-point--approved-by-amino-2026-08-21)'s
  pre-agreed split point, so if the Step has to shed a Sub-step this is the one.
  **Three things 4.5 should read before it starts.** `ENTRY_KINDS` in
  `veritas/semantic/loader.py` still refuses `semantic/dimensions/` and now says so in
  the second person — 4.4 added its own row there and the comment records that 4.5
  comes here too, because the mapping is not a scan of the tree. The loader already
  reads booleans the **YAML 1.2** way for 4.5's benefit specifically: a Dimension
  Definition's allowed values are domain text, and YAML 1.1 turns `no`, `on`, `y` and
  `n` into booleans in any casing — Norway's country code and Ontario's are both in
  that set. And
  ...
  binds the date axis: it is deliberately **not** a calendar boundary for Snapshot
  metrics, and 4.5 owes
  [DEBT-012](../debt-ledger.md#debt-012--the-price-table-is-sparse-so-the-snapshot-calendar-has-holes)
  a dated status note recording the deferral, *"since a deferral nobody wrote down is
...
**The Semantic Layer is most of the way built.** Sub-step 4.1 published one Metric
Definition and the Join Path it carries, in the format the Step 004 plan proposed and
Amino approved, and proved the arrangement end-to-end: the expression the file
publishes is pasted verbatim into a query built from the entry's own Join Path and
date column, executed through the Warehouse Adapter, and returns the same figure
`check_warehouse.py` computes from SQL that never reads `semantic/`. 4.2 filled that
shape out to all nine Certified Metrics and the eight Join Paths they are computed
across, and **4.4 added the Ambiguous Terms** — the five words of
...
matters: that 'revenue' has two certified meanings"*. **Three entry types of four, and
all three complete**; Dimension Definitions are the fourth and are Sub-step 4.5.
Nothing above the Semantic Layer is built: no Retrieval, no Orchestrator, no
application, and nothing yet turns a question into SQL.
...
**The Semantic Layer is two entry types of four.** `semantic/metrics/` and
`semantic/joins/` are complete; there is no Ambiguous Term, so nothing in the
...
be sliced. Both are Sub-steps 4.4 and 4.5. Three things the format carries are still
published and unchecked — `description`, `grain` and `aliases`, which are what
Retrieval will match on when Retrieval exists. `derives_from` left that list in 4.2:
it is now read, and what reads it is the check that assembles a composed metric's
query.

**A Snapshot metric executed over the whole Warehouse is not a number anyone would
ask for.** `Cash Balance` summed across every Snapshot date is 640 dates added
together, and the check executes it that way on purpose — it is the strongest thing a
...
**9 open debt** — see [debt-ledger.md](../debt-ledger.md) — plus **4 paid**, 1

...
**Last updated:** 2026-08-30 — **Step 005 — Build the Validation Gate — is `done`, and
...
**`tests/` holds Delivery Mode's two guards, the six corpus checks and the search
checks**; `uv run pytest` is the command that proves behaviour from here on.

---

## Resume here

- **Next: Sub-step 6.3 — resolve Ambiguous Terms before retrieval.** The
  [Step 006 plan](../plan/step-006-retrieval-and-orchestrator.md) is `active`, and
  Sub-step 6.2 is built and waiting on Amino's commit. 6.3 is the first Large
  Language Model (LLM) call, and it chooses the provider and the model; nothing in
  the repository talks to one yet.
...
|---|---|---|
| Python environment | ✅ working | `uv`-managed, CPython 3.14.4, `.venv/`. `pyproject.toml` + `uv.lock` + `.python-version` all pinned. **Six declared dependencies**: `duckdb`, `dlt`, `sqlglot`, `pyyaml`, and — since Sub-step 6.2 — `minsearch` and `fastembed`. dlt brings the bulk of the transitive tree; `fastembed` brings `onnxruntime` and `minsearch` brings `scikit-learn`, `numpy` and `pandas`. Two check scripts are standard-library-only — `verify_framework.py` and `check_data_availability.py`; the other four and the `check_validation_gate/` package import third-party code. Everything imported anywhere is one of the six declared libraries. |
...
rendered as text a search could match. **Nothing searches it** — there is no index, no
query and no embedding — and nothing turns a question into SQL. The machine that chooses
...
would **write** such a query: nothing that searches the corpus to find the axis, no
Grounding to put it in a prompt, and no generator.
...
**Open debt: 16.** The newest is
[DEBT-027](../debt-ledger.md#debt-027--the-searchable-text-is-one-flat-field-so-a-name-match-cannot-outrank-a-description-match), which is where 6.1's flat `text` field stops being a note in a review:

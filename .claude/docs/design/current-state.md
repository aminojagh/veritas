...
**Last updated:** 2026-08-23 — **the scan reads the corpus.** Sub-step 4.3 of Step 004 is written and verified: `check_warehouse.py`'s `check_seam` now reads every piece of SQL the Semantic Layer publishes as well as the SQL a module emits, and reads all of it **twice** — by function name, as before, and by **type**, retargeting each statement to BigQuery and reporting every type construct that arrives there saying less than it says at home. [DEBT-015](../debt-ledger.md#debt-015--the-dialect-scan-names-functions-and-the-loss-measured-was-in-a-cast) is **paid**, [ADR-0002](../adr/0002-duckdb-as-the-warehouse-behind-an-adapter.md#status-note-2026-08-23--the-mitigation-now-says-construct-and-a-run-performs-it)'s mitigation says *construct* where it said *function*, and the open-debt count is back to **9**. **The corpus itself did not change**: `semantic/metrics/` publishes all nine Certified Metrics of Glossary Section B, `semantic/joins/` the eight Join Paths they are computed across, every one of the nine executes against the real Warehouse, and every one agrees with a figure `check_warehouse.py` works out for itself. **The third of nine components is under construction — two entry types of four, and both of them complete.** Step 004 is `active`, approved by Amino on 2026-08-21 with all seven of its rulings, amended on 2026-08-22 by [R8](../plan/step-004-semantic-layer.md#r8--the-route-a-metric-definition-carries--decided-in-sub-step-42-under-aminos-ruling-of-2026-08-22), which settled the `join_path` question at the start of 4.2 as Amino ruled it should be, and amended again on 2026-08-23 by [R9](../plan/step-004-semantic-layer.md#r9--aminos-four-rulings-on-the-42-review--decided-2026-08-23), which records Amino's four rulings on the 4.2 review; **Sub-step 4.4 is the next commit.** **The Warehouse is full: all ten tables of Glossary Section B hold rows, every Certified Metric can return a number, and the adapter seam is checked in all three of the readings ADR-0002's two signals now come to.**

**Steps completed:** Step 000 (framework), Step 001 and **Step 002, all six Sub-steps, fully committed**. Step 000 and Sub-step 1.1 in `6281e6b`, Sub-step 1.2 in `4b48a46`, Sub-step 1.3 in `9c5b060`, Step 002 planning in `57e8aee`, Sub-step 2.1 in `5a061a7`, the R16 plan amendment in `cd5e7dd`, Sub-step 2.2 in `0fc5a34`, Sub-step 2.3 in `a58ef91`, Sub-step 2.4 in `13b99bb`, Sub-step 2.5 in `ce2961a`, Sub-step 2.6 in `6a16d3d`, Step 003 planning in `40d72d8`, **Sub-step 3.1 in `d840fa8`, Sub-step 3.2 in `89fee55`, Sub-step 3.3 in `23020e9`, Sub-step 3.4 in `c20d601`, Sub-step 3.5 in `fcf4b7d`**. **Step 004 planning is committed in `5d95393`** and **Sub-step 4.1 in `6c15736`**, which is what wrote 3.5's hash above and turned the [Step 003 plan](../plan/step-003-validation-feasibility.md) from `in review` to `done` — how every hash above arrived, and how `40d72d8` closed Step 002. **Sub-step 4.2 is committed in `333d6fc`. This commit is Sub-step 4.3**; its own hash is not here for the reason every Sub-step's is not, which is that this file is part of it, and the Sub-step 4.4 commit fills it in.

---

## Resume here

- **Next: Sub-step 4.4 — write the Ambiguous Terms.**
  [The plan](../plan/step-004-semantic-layer.md#44--write-the-ambiguous-terms)
  asks for the five rows of
  [Glossary Section D](../glossary.md#d-ambiguous-terms) as `semantic/ambiguous/`
  entries — "revenue", "volume", "balance", "P&L", "how much does X have" — each
  naming the Certified Metrics it disambiguates between through the `disambiguates`
  field [EXT-005](../extension-register.md#ext-005--semantic-layer-coherence-checks)
  already chose, plus EXT-005's fourth rule in `check_semantic_layer.py`: **every
  metric an Ambiguous Term names must exist as a Metric Definition.** Its
  verification is `uv run python .claude/scripts/check_semantic_layer.py` plus the
  mutation the plan names — point one Ambiguous Term at a metric that does not
  exist, see the run fail, restore, `cmp`.
  **This is the Sub-step carrying the project's central claim.** ADR-0001 rejected
  ...
  retrievable.
  **Two things 4.4 should read before it starts.** The loader knows two entry
  kinds, `metrics` and `joins`, and `ENTRY_KINDS` in `veritas/semantic/loader.py`
  says in a comment that the remaining two are *"absent on purpose — a file in a
  directory this mapping does not know fails to load rather than being skipped, so
  the Sub-step that writes the first Ambiguous Term has to come here and say so"*.
  And finding 2 below is a decision made in 4.2 that nothing currently checks: **no
  metric's `aliases` contain an Ambiguous Term**, and the check that would enforce
  it belongs here.
  **One question is open for Amino, and it does not block 4.4.** [R9](../plan/step-004-semantic-layer.md#r9--aminos-four-rulings-on-the-42-review--decided-2026-08-23) renamed two
  Join Paths on his ruling, and the ruling wrote the second name with its prefix elided —
  `..._on_quotation_currency`. It was resolved to `instrument_to_fx_rate_on_quotation_currency`,
  keeping every name under `semantic/joins/` starting with its own `from_table`; R9's
  third ruling argues why and says what the other reading costs, which is one more rename
  and one `from_table` field.

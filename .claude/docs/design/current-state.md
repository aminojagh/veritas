...
**Last updated:** 2026-09-03 — **Step 007 — Evaluation — is `done`, and so is every Step
...
not only in the registered name. **Step 008 — Observability — is `active`, and three of
...
`docker compose up -d postgres` is the server, and the App says in its sidebar whether it
is recording.
...
**Sub-step 8.3 built the Question Log.** `veritas/observability/` is the seam, three
...
rendering, page and recording checks, the gold checks, the evaluation checks and the
Question Log checks**;
`uv run pytest` is the command that
proves behaviour from here on, and no test in it needs a key or a network — the four that
call a real provider run only when `VERITAS_LIVE_MODEL` is set, and the eight that need a
Postgres server skip without one.

---

## Resume here

- **Sub-step 8.3 is approved and staged for commit; 8.1 (`2cf4170`) and 8.2 (`b75bdda`)
  are committed. Next is Sub-step 8.4 — leave Feedback on a Grounded Answer.** 8.3 built
  `veritas/observability/`, the compose file and the Postgres credentials in
  `.env.example`, and paid
  [DEBT-032](../debt-ledger.md#debt-032--a-refusal-that-is-not-the-gates-carries-no-reason-a-chart-can-group-by):
  `EndedBy` is the Grounded Answer's own field, `no sql` split into `retrieval` and
  `generation`, and `endings()` holds what a producer says against what the fields show.
  `ended_by` is stated by the producer rather than derived, which is why
  `GroundedAnswer` requires it; the
  [8.3 review](../reviews/step-008-observability.md#sub-step-83--record-every-question-a-person-asks)
  argues that and seven other points, all approved on 2026-09-04. Two entries
  opened, both foreseen by the plan:
  ...
  **failed its own runner** — Groq's free tier is capped at 200,000 tokens per day, the
  ...
  on 2026-09-03, five Sub-steps: 8.1, 8.2 and 8.3, all three built and described above;
  Feedback in the App, written against the row 8.3 records; and the Grafana dashboard,
  seven charts over those rows, which also gains Grafana in the compose file.
...
- **No Term Proposal is open**: the plan's two —
  ...
  **`Gold Question`** and **`Relevant Set`** were agreed on 2026-09-01, and 7.2's
  Section D column the same day. **The debt picture after 8.3: fifteen open, twenty-two
  paid, two accepted, two moved.**
  [DEBT-032](../debt-ledger.md#debt-032--a-refusal-that-is-not-the-gates-carries-no-reason-a-chart-can-group-by)
  is **paid** by 8.3 and
  [DEBT-034](../debt-ledger.md#debt-034--lineage-records-what-the-model-was-shown-not-what-the-statement-used)
...
over a committed Gold Question Set, and Observability writes every question down. What
the ninth still lacks is the charts over those rows and the Feedback widget beside the
answer — Sub-steps 8.4 and 8.5.
...
| Containerization | ◐ one of three services | `docker-compose.yml` — **Postgres only**, arrived with the Question Log in Sub-step 8.3 because the component that writes to a server has to be able to start one. The [Target State row](target-state.md#components) names three services: `app`, Postgres and Grafana. Grafana is Sub-step 8.5's and the App's own image is Step 009's, and until it exists the App runs on the host and reaches the server over the published port, which is why `POSTGRES_HOST` is a variable and defaults to `localhost`. |
...
│   ├── evaluation/
│   │   ├── __init__.py        # re-exports both modules
│   │   ├── __main__.py        # `-m veritas.evaluation retrieval` — the sweep, as a table
...
BY`s away rather than chart descriptions. What is missing is the charts and the Grafana
service, which are Sub-step 8.5, and Feedback, which is 8.4: the `question` table has no
verdict column yet, so the fourth of an
[`Operational Measure`](../glossary.md#a-the-system)'s four is still unrecorded. Three
...
in `cf64d28`, **Step 008 planning in `874afe9`**, Sub-step 8.1 in `2cf4170`, and Sub-step
8.2 in the commit that carries this line. A Step's planning commit is normally what

...
decision. **Step 006 is `active` and its first Sub-step is built**: `veritas/retrieval/`
renders every Semantic Entry as the text a search may match, and renders a Join Path as
nothing, because everything a Join Path says is a Warehouse identifier. **`tests/` holds
Delivery Mode's two guards and the six corpus checks**; `uv run pytest` is the command
that proves behaviour from here on.

---

## Resume here

- **Next: Sub-step 6.2 — retrieve Semantic Entries for a question.** The
  [Step 006 plan](../plan/step-006-retrieval-and-orchestrator.md) is `active`, and
  Sub-step 6.1 is built and waiting on Amino's commit. 6.2 puts the seam
  `retrieve(question) -> list[SemanticEntry]` in front of the searchable text 6.1
  produces, under three strategies Step 007 will measure against each other. It
  needs a search library and an embedding model, and neither is declared yet —
  `pyproject.toml` still names four dependencies.
...
| Retrieval | 🚧 partial | `veritas/retrieval/` — `searchable.py` behind an `__init__.py` that re-exports it, laid out like `veritas/semantic/`. `SEARCHABLE_FIELDS` is the whitelist of fields a search may match, one row per entry type and disjoint from the loader's `SQL_FIELDS`; `searchable_entries()` renders the whole Semantic Layer as one record per entry — `name`, `kind`, and the single `text` field a search reads. A **Join Path renders as empty text**, because its name, its two tables and its `on` clause are Warehouse identifiers end to end; it is reached by reference from the Metric Definitions and Dimension Definitions that name it. **Nothing searches yet** — no index, no query, no embedding, and no dependency for any of them. |
| Orchestrator | ✗ none | — |
...
│   ├── retrieval/
│   │   ├── __init__.py        # re-exports searchable.py

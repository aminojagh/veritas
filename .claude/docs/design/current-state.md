...
**Sub-step 6.3 built the first model call**: `veritas/llm/` is the one place a
...
[ADR-0005](../adr/0005-one-openai-compatible-endpoint-for-every-provider.md) — and
`veritas/orchestrator/rewrite.py` resolves the Ambiguous Terms a question says
against those entries' own resolution rules, or asks back which meaning was meant.
**`tests/` holds Delivery Mode's two guards, the six corpus checks, the search checks
and the rewrite and boundary checks**; `uv run pytest` is the command that proves
behaviour from here on, and no test in it needs a key or a network — the one test
that calls a real provider runs only when `VERITAS_LIVE_MODEL` is set.

---

## Resume here

- **Next: Sub-step 6.4 — answer a question end-to-end.** The
  [Step 006 plan](../plan/step-006-retrieval-and-orchestrator.md) is `active`, and
  Sub-step 6.3 is built and waiting on Amino's commit. 6.4 grounds from retrieved
  entries, generates SQL, validates it, executes it, and returns a Grounded Answer
  carrying its Lineage and Validation Gate outcome.
...
means. Retrieval searches it; nothing above Retrieval is built — no Orchestrator, no
...
| Orchestrator | ◐ one step of seven | `veritas/orchestrator/` — `rewrite.py` behind an `__init__.py` that re-exports it. **REWRITE only**; the sequence itself, GROUND, GENERATE and ANSWER do not exist. `rewrite(question)` returns a frozen `Rewrite` carrying the question as asked, the text Retrieval searches, the Certified Metrics each Ambiguous Term resolved to, and the clarification asked back — `None` exactly when the question is ready to retrieve for. Which terms a question says is matched here against the names [Glossary Section D](../glossary.md#d-ambiguous-terms) registers, in the order the question says them, with `X` in a name matching the subject it stands for; which meaning it named is the model's answer, given only those terms' own `description` and `resolution`; whether that answer counts is decided here again, against `disambiguates`. A question saying no Ambiguous Term costs no model call at all. **Two things this does not do**, both open: detection is the registered spelling and nothing else, so *"revenues"*, *"PnL"*, *"turnover"* and *"how much is in"* reach Retrieval as though they had been unambiguous ([DEBT-029](../debt-ledger.md#debt-029--ambiguous-term-detection-is-literal-so-every-other-phrasing-of-a-registered-word-passes-silently), pinned by a test that asserts today's misses); and a resolved meaning is appended to the question rather than spliced over the word, which is the text every Retrieval Strategy searches and is unmeasured ([DEBT-030](../debt-ledger.md#debt-030--the-resolved-meaning-is-appended-to-the-question-and-nothing-has-measured-that-against-splicing-it)). |
...
│   ├── orchestrator/
│   │   ├── __init__.py        # re-exports rewrite.py
...
Gold Question Set is what would. A Metric Definition's `grain` is read by nobody beyond
...
does with each answer a model could give. **One test is not a stub, and it is opt-in**:
`VERITAS_LIVE_MODEL=1` runs the configured provider against the real prompt, which is how
[DEBT-028](../debt-ledger.md#debt-028--no-test-reaches-a-real-provider-so-the-live-path-is-proven-only-by-a-stub-server)
was paid. It has been run against both providers — a key sitting in `.env` for the App is
not consent to spend it on every run, which is why it stays opt-in.
...
share a trigger — the Sub-step that builds Grounding, which is 6.4 — and a home in
`route_of_resolved`. Neither is demonstrated by anything that runs, and each owes a probe
from the Sub-step that pays it.

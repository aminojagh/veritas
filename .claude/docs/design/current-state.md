...
**Last updated:** 2026-08-23 — **the corpus exists.** Sub-step 4.2 of Step 004 is written and verified: `semantic/metrics/` publishes **all nine Certified Metrics of Glossary Section B**, `semantic/joins/` publishes the eight Join Paths they are computed across, every one of the nine executes against the real Warehouse, and every one agrees with a figure `check_warehouse.py` works out for itself. **The third of nine components is under construction — two entry types of four, and both of them complete.** Step 004 is `active`, approved by Amino on 2026-08-21 with all seven of its rulings, amended on 2026-08-22 by [R8](../plan/step-004-semantic-layer.md#r8--the-route-a-metric-definition-carries--decided-in-sub-step-42-under-aminos-ruling-of-2026-08-22), which settled the `join_path` question at the start of 4.2 as Amino ruled it should be, and amended again on 2026-08-23 by [R9](../plan/step-004-semantic-layer.md#r9--aminos-four-rulings-on-the-42-review--decided-2026-08-23), which records Amino's four rulings on the 4.2 review — the only one that moved a file renamed two Join Paths onto the currency axis, in this same commit; **Sub-step 4.3 is the next commit.** **The Warehouse is full: all ten tables of Glossary Section B hold rows, every Certified Metric can return a number, and the adapter seam is checked in both the halves ADR-0002 named. The sqlglot spike has answered all four of its claims and the go/no-go is now recorded: `.claude/docs/design/validation-feasibility.md` exists and its verdict is GO on ADR-0003, with six constraints on the Step that builds the Semantic Layer.**
**Steps completed:** Step 000 (framework), Step 001 and **Step 002, all six Sub-steps, fully committed**. Step 000 and Sub-step 1.1 in `6281e6b`, Sub-step 1.2 in `4b48a46`, Sub-step 1.3 in `9c5b060`, Step 002 planning in `57e8aee`, Sub-step 2.1 in `5a061a7`, the R16 plan amendment in `cd5e7dd`, Sub-step 2.2 in `0fc5a34`, Sub-step 2.3 in `a58ef91`, Sub-step 2.4 in `13b99bb`, Sub-step 2.5 in `ce2961a`, Sub-step 2.6 in `6a16d3d`, Step 003 planning in `40d72d8`, **Sub-step 3.1 in `d840fa8`, Sub-step 3.2 in `89fee55`, Sub-step 3.3 in `23020e9`, Sub-step 3.4 in `c20d601`, Sub-step 3.5 in `fcf4b7d`**. **Step 004 planning is committed in `5d95393`** and **Sub-step 4.1 in `6c15736`**, which is what wrote 3.5's hash above and turned the [Step 003 plan](../plan/step-003-validation-feasibility.md) from `in review` to `done` — how every hash above arrived, and how `40d72d8` closed Step 002. **This commit is Sub-step 4.2**; its own hash is not here for the reason every Sub-step's is not, which is that this file is part of it, and the Sub-step 4.3 commit fills it in.

---

## Resume here

- **Next: Sub-step 4.3 — pay DEBT-015: the dialect scan reads type constructs.**
  [The plan](../plan/step-004-semantic-layer.md#43--pay-debt-015-the-dialect-scan-reads-type-constructs)
  asks for three things in one commit: `check_warehouse.py`'s `check_seam` reads
  Metric Definition expressions out of `semantic/` as well as the SQL `veritas/`
  emits and flags dialect-shaped **type** constructs by comparing the parse tree
  before and after retargeting — the `round_trip_rewrites` instrument Sub-step 3.4
  already committed;
  [ADR-0002](../adr/0002-duckdb-as-the-warehouse-behind-an-adapter.md)'s mitigation is
  reworded from *function* to *construct* with a dated note and no status change; and
  the Ledger entry is marked **paid**. Its verification is
  `uv run python .claude/scripts/check_warehouse.py` plus the mutation the entry
  names: the run must **name** `Traded Notional`'s cast where `HEAD` says nothing,
  because *"a scan that flags nothing after this change has not been paid, it has
  been re-promised."*
  **One thing 4.3 should read before it starts, and it makes the Sub-step wider than
  ...
  names one cast — *"`Traded Notional`'s"* — and the corpus carries the same widening
  cast on **every** expression whose product overflows `DECIMAL(18)`, which is every
  monetary metric on the Snapshot side plus the ledger. `check_semantic_layer.py`
  prints them on every run, under *widening cast*, by executing each expression with
  its cast taken back out and showing the engine's refusal. So the scan 4.3 fixes has
  more than one construct to find, and the
  [4.2 review](../reviews/step-004-semantic-layer.md#sub-step-42--write-the-remaining-metric-definitions)
  says so where the block is quoted.
  **One question is open for Amino, and it does not block 4.3.** [R9](../plan/step-004-semantic-layer.md#r9--aminos-four-rulings-on-the-42-review--decided-2026-08-23) renamed two
...
- **What Sub-step 4.2 built**, and what it deliberately did not. `semantic/metrics/`
  ...
  **every** metric now has a second opinion and R2's weaker branch is unreachable —
  ...
  no Access Profile, and `check_warehouse.py`'s dialect scan still does not read
  `semantic/` — that is 4.3.
